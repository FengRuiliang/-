...
MSAABB::MSAABB(const Vec3f& max, const Vec3f& min)
{
	set(max, min);
}

MSAABB::MSAABB(const MSAABB& box)
{
	set(box._min, box._max);
}

Vec3f MSAABB::getCenter()
{
	Vec3f center;
...
void MSAABB::getCorners(Vec3f *dst) const
{
	assert(dst);
...
bool MSAABB::intersects(const MSAABB& aabb) const
{
...
bool MSAABB::containPoint(const Vec3f& point) const
{
...
void MSAABB::merge(const MSAABB& box)
{
...
void MSAABB::set(const Vec3f& max, const Vec3f& min)
{
	this->_min = min;
	this->_max = max;
}
...
bool MSAABB::isEmpty() const
{
	return _min.x() > _max.x() || _min.y() > _max.y() || _min.z() > _max.z();
}

void MSAABB::updateMinMax(const Vec3f point)
{
		// Leftmost point.
		if (point.x() < _min.x())
			_min.x() = point.x();
...
//void MSAABB::transform(const Mat4& mat)
//{
//	Vec3f corners[8];
