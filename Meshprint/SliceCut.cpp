...
void SliceCut::write_contour_to_txt(const char*  byfilename,int slice_id)
{
	std::ofstream fout(byfilename);
	...
	for (int i=0;i<num_pieces_;i++)
	{
		...
		for (int j=0;j<pieces_list_[i].size();j++)
		{
			...
			{
				fout << j << " " << k << " " << pieces_list_[i][j]->at(k).position_vert[0].x() << " " <<
					pieces_list_[i][j]->at(k).position_vert[0].y() << " " << "\n";
					//pieces_list_[i][j]->at(k).position_vert[0].z() << "\n";
			}
		}
	}
	fout.close();
			...
			else
			{
				...
				if (cur_edge_ != NULL)
				{
					...
					do
					{
						...
						cutLine temp_cut_line(pos1, pos2);
						temp_cut_line.angle_belong_facet = acos(faces.at(*iter)->normal().dot(Vec3f(0, 0, -1))) * 180 / 3.1415926;
						circle_list_->push_back(temp_cut_line);
